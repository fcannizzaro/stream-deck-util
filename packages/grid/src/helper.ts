...
type Hooks = {
  onPreRender?: (cells: Cell[]) => void;
  onPostRender?: (cells: Cell[]) => void;
};
...
export class GridHelper {
   ...
   */
  public reload() {
    this.hooks.onPreRender?.(this.cells);
    ...
    this.hooks.onPostRender?.(this.cells);
  }
  ...
  onDial(e: ActionEvent<any>, clockwise: boolean) {
    ...
    if (clockwise && btn.onDialRight) {
      return btn.onDialRight();
    }
    if (!clockwise && btn.onDialLeft) {
      return btn.onDialLeft();
    }
